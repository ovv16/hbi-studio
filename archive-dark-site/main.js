...
(() => {
  ...
  if (ring && dot && window.matchMedia('(pointer: fine)').matches) {
    ...
    lerp();
    const hoverables = 'a, button, .gallery-tile, .svc-card, .ba-handle, .ba-knob, input, textarea';
  ...
  const BG_PRESETS = {
    jet:      { ink: '#0D0B09', ink2: '#100E0B', panel: '#15120F', card: '#1C1915', warm: '#241C14' },
    warm:     { ink: '#15110D', ink2: '#181410', panel: '#1B1612', card: '#241D17', warm: '#2C2317' },
    charcoal: { ink: '#0B0B0C', ink2: '#0E0E0F', panel: '#131314', card: '#191919', warm: '#1F1C18' }
  };
  const applyBg = (k) => {
    const p = BG_PRESETS[k] || BG_PRESETS.jet;
